document.addEventListener("DOMContentLoaded", () => {
    ...
    uploadForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success) {
            // Update the video stream with the uploaded video URL
            videoStream.src = data.video_url + "?" + new Date().getTime()
            showStatus(uploadStatus, "Video uploaded and processing started", "success")
          } else {
  ...
  // Handle video stream loading events
  if (videoStream) {
    videoStream.addEventListener("error", () => {
      hideSpinner()
      showStatus(uploadStatus, "Error loading video stream", "error")
    })
